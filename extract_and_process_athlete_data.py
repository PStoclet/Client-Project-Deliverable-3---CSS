...
zip_file_path = 'C:/Users/pablosto/Desktop/athletes.zip'
...
# Step 3: Iterate over all athletes in both teams
team_folders = ['mens_team', 'womens_team']
for team_folder in team_folders:
    athlete_files = [f for f in os.listdir(os.path.join(extract_dir, team_folder)) if f.endswith('.csv')]
    for athlete_file in athlete_files:
        csv_path = os.path.join(extract_dir, team_folder, athlete_file)
        process_athlete(csv_path)

# Step 4: Save athlete data as JSON
with open('athlete_datawomen.json', 'w', encoding='utf-8') as json_file:
